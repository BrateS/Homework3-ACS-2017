...
void check_adj_pixels(int width, int height, pixel **img, pixel pixel,
                      int threshold, int i, int j, int *n, pos **positions,
...
void write_bin(pixel **img, char *input_file) {
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      if (marked[i][j] == 0) {
        int n = 1;       // nr ele vector positii
        int counter = 1; // trece prin vector
        ...
        while (counter <= n) {
          ...
          check_adj_pixels(width, height, img, 
			   img[i][j], threshold,line, col,
                           &n, &positions, marked);
          counter++;
        }
        zones++;
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      ...
      } else if ((pixel_within_threshold(img[i][j], img[i + 1][j], 0) == 0) ||
                 (pixel_within_threshold(img[i][j], img[i][j + 1], 0) == 0)) {
        j++;
        i++;
        fwrite(&i, sizeof(short), 1, out);
        fwrite(&j, sizeof(short), 1, out);
...
int pixel_within_threshold(pixel pixel1, pixel pixel2,
				 int threshold) {
  if (threshold == 0) {
    if (pixel_eq(pixel1, pixel2) == 1)
      return 1;
  } else {
    if (abs(pixel1.r - pixel2.r) + (abs(pixel1.b - pixel2.b)) +
